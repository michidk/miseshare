...
- Stream audio is kept on a separate native WebRTC media track. Publishers can add screen audio or microphone input without restarting the screen capture; when both are active, the browser mixes them into the same outgoing track. Each source remains independently controllable, and every receiver can mute each incoming stream.
